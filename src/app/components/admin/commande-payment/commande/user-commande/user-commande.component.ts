...
getCommandesNonPayer(){
  this.commandeUserService.getCommandesUserNonPayer(this.user.id).subscribe((data: Array<Commande>) => {
...
getCommandesPayer(){
  console.log("data");
  this.commandeUserService.getCommandesUserPayer(this.user.id).subscribe((data: Array<Commande>) => {
  ...
  enregistrerPayment() {
    ...
    this.paymentUserService.postPayment(this.payment).subscribe((data: Payment) => {
      ...
      this.modalService.info({
        ...
        nzOkText: null,
        nzCancelText: 'Ok',
        nzOnCancel: () => this.finPayment(data.commande.id)
      });
  ...
  finPayment(id:number){
    this.commandesNonPayerList = this.commandesNonPayerList.filter(d => d.id !== id);
  ...
  supprimerCommande(commande: Commande) {
    this.commandeUserService.deleteCommandeUser(commande.id).subscribe((data: Boolean) => {
      console.log(data);
