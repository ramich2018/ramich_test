...
@Component({
  selector: 'app-acceptation-demande-service',
  templateUrl: './acceptation-demande-service.component.html',
  styleUrls: ['./acceptation-demande-service.component.css']
})
export class AcceptationDemandeServiceComponent implements OnInit {
  ...
  serviceDemandeList = [];
  serviceAccepterList = [];
  serviceRejeterList: Array<Publication> = [];
  serviceCreerList: Array<Publication> = [];
  ...
  rejeterService(key: string): void {

    this.publicationService.putrejetService(key, this.editCache3[key].data)
      .subscribe(
        res => {
          console.log(res);
          this.ngOnInit();
  ...
  updateEditCache3(): void {
    this.serviceDemandeList.forEach(item => {
  ...
  loadDemandesPubNAc() {
    this.publicationService.getDemandesPubService()
      .subscribe((data: Array<Publication>) => {
        this.serviceDemandeList = data;
        this.dat3 = data;
        this.updateEditCache3();
        console.log(this.serviceDemandeList);
      }, err => {
        console.log(err);
      })
  }
  ...
  loadDemandesPubNC() {
    this.publicationService.getDemandesServiceAccepter()
      .subscribe((data: Array<Publication>) => {
        this.serviceDemandeList = data;
        this.dat4 = data;
  ...
  search3(): void {
    ...
    this.serviceDemandeList = data.sort((a, b) => (this.sortValue3 === 'ascend') ? (a[this.sortName3] > b[this.sortName3] ? 1 : -1) :
  ...
  search4(): void {
    ...
    this.serviceDemandeList = data.sort((a, b) => (this.sortValue4 === 'ascend') ? (a[this.sortName4] > b[this.sortName4] ? 1 : -1) :
  ...
  isEchang() {
    if (this.inputValue == 'ECHA') return true;
  }
  ...
  ngOnInitUpdate4(key: string) {
    ...
    this.validateForm = this.fb.group({
      ...
      tBCCV: [this.editCache4[key].data.tBCCV, [Validators.required]],
  ...
  getDemandeServiceRejeter() {
    this.publicationService.getDemandesServiceRejeter().subscribe(
      (data: Array<Publication>) => {
        this.serviceRejeterList = data;
